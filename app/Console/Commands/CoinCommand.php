...
class CoinCommand extends Command
{
     ...
     * @var string
     */
    protected $signature = 'app:coin-command';
    ...
    public function handle()
    {
        ...
        // file_put_contents('/home/astj9531/public_html/mycrypto.astodigi.com/laravel/aaDatalast1.txt', print_r($currentCoins, true));
        
        foreach ($currentCoins as $coin) {
            $symbol = $coin['symbol'];
            $existingCoin = Coin::where('symbol', $symbol)->first();
            

            if ($currentHour == 7) {
                $history = History::where('symbol', $symbol)->whereNull('price_close')->whereNull('price_hit_25')->get();
                foreach ($history as $his) {
                    if ($his) {
                        $his->update(['price_close', 11111]);
                    }
                }
